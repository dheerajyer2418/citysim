...
This is the plan to generalize CitySim from one community area to **all 77 Chicago community areas**.

**STATUS (updated):** The pipeline is area-aware and the **needs map (`s7`) is built and deployed for all 77
community areas** — Steps 1-3 below are effectively DONE. The MATSim traffic simulations / BCA (Step 4)
remain incremental and are validated for Logan Square (community area **22**) plus a handful of others.
See "Chicago-wide Needs Map" in `CLAUDE.md` for the helper scripts (`scripts/gen_area_configs.py`,
`scripts/build_area_needs.py`) and the lazy-loading site build.

Live demo: https://citysim-five.vercel.app/
...
- [x] Add `areas` config + `area_slug`/`scenario_dir` helpers on `CitySimConfig`.
- [x] Thread `--area` through `cli.py` and every `pipeline/*.py` `run()`.
- [x] Parameterize the `viz/*.py` builders and `scenario_server.py`.
- [x] Verify Logan Square still reproduces (regression).
- [x] Fetch all 77 community areas; build boundary+network+needs index for each (batch, no sim). Driver: `scripts/build_area_needs.py` (`s0 -> s1 -> s7`). Config generated by `scripts/gen_area_configs.py`.
- [x] Multi-area static site (area picker) + Vercel deploy. Unified maps lazy-load per-area data (`public/<slug>/data/needs_payload.json`) to stay under GitHub/browser size limits.
- [ ] Per-area MATSim scenarios, added incrementally (Step 4 — still open; only ~10 areas have sims).
- [ ] Per-area calibration of cordon scaling / stuckTime.
- [ ] Apply the same lazy-load treatment to `public/live_traffic.html` (still inlines its sim areas, ~55 MB) before many more areas get MATSim sims.
